...
### Bronze
- Ingesta cruda desde la API, sin transformaciones
- Se agrega timestamp de ingesta (`ingested_at`)
- Se acumulan snapshots historicos (append)

### Silver
- Limpieza: nulos, duplicados, tipos de datos
- Normalizacion y estandarizacion
- Validaciones de calidad

### Gold
- Tablas agregadas orientadas a negocio
- Modelo dimensional (hechos + dimensiones)
- Estrategia **delete-insert por dia**: se recalcula solo el dia actual, el historico queda fijo
...
4. **Dashboard en Streamlit** con al menos:
   - Una vista por capa (Bronze, Silver, Gold)
   - Indicador de ultima actualizacion de datos
   - Filtros interactivos (fecha, categoria, etc.)
...
Para cada API se detalla como aplicar la arquitectura medallion. Solo se incluyen APIs con datos que se actualizan al menos cada hora, lo que justifica tener Airflow corriendo y acumulando snapshots continuamente.
