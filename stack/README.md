...
El dashboard de Streamlit arranca vacío (placeholder). Las páginas se agregan a partir de la **clase 03** (Bronze), **clase 04** (Silver) y **clase 05** (Gold) — al guardar archivos en `stack/dashboard/pages/`, Streamlit los detecta automáticamente y aparecen en el menú lateral.
...
├── dashboard/                  # Dashboard Streamlit (placeholder, se llena en clase03+)
│   ├── Dockerfile              # Imagen del dashboard
│   ├── app.py                  # Pagina principal (placeholder de bienvenida)
│   ├── db.py                   # Helpers de conexion a Postgres (engine, table_exists, ...)
│   ├── pages/                  # Paginas Streamlit (vacias por ahora)
